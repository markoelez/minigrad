...
from minigrad.util import DEBUG, log, topological_sort
...
class Tensor:
  ...
  def randn(cls, *shape, **kwargs):
    return cls(np.random.randn(*shape).astype(np.float32), **kwargs)  # type: ignore

  @classmethod
  def uniform(cls, *shape, **kwargs):
    return cls((np.random.uniform(-1.0, 1.0, size=shape) / np.sqrt(np.prod(shape))).astype(np.float32), **kwargs)
  ...
  def __div__(self, x: Tensor) -> Tensor:
    return self.div(x)

  def __add__(self, x: Tensor) -> Tensor:
    return self.add(x)

  def __sub__(self, x: Tensor) -> Tensor:
    return self.sub(x)
